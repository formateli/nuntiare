...
class UITabs(ttk.Frame):
    ...
    def __init__(self, parent, style=None):
        UITabs._get_style()
        if style is None:
            self._curr_style_class = UITabs._style_class
        else:
            self._curr_style_class = style

        super(UITabs, self).__init__(parent)
        self.parent = parent
    ...
    def bind(self, event, callback):
        if event in self._bind_callbacks:
            ...
            super(UITabs, self).bind(event, callback)
    ...
    def remove(self, tabid):
        ...
        if tabid == self.current:
            ...
            if self.current is not None:
                self._fire_event('tab_selected', self, self.current)
    ...
    def select(self, tabid):
        ...
        if self.current != tabid:
            self._fire_event('tab_deselected', self, self.current)
            self.current = tabid
            self.rebuild()
            self._fire_event('tab_selected', self, self.current)
    ...
    def define_appearance(self):
        style = ttk.Style()
        UITabs._validate_style(self._curr_style_class)
    ...
    def addtab(self, ev=None):
        self._fire_event('tab_added', self, None)

    def closetab(self, tabid, ev):
        self._fire_event('tab_closed', self, tabid)
    ...
    def _fire_event(self, event, tabs, tabid):
    ...
    def _validate_style(cls, name):
        style = ttk.Style()
        add_opts = {}
        for opt, value in UITabs._style_opts.items():
    ...
    @classmethod
    def _get_style(cls):
        if UITabs._style_class is not None:
            return

        UITabs._style_class = 'UITabs'
        ...
        UITabs._style_opts = {
                ...
                'gapwidth': 15,
                'background': '#c4c4c4',
                'selectedbackground': '#d3d3d3',
                'dividerbackground': '#b0b0b0',
                'textcolor': '#424242',
                'holecolor': '#eeeeee',
            }


class UITabsTheme(UITabs, WidgetThemeMixin):
    def __init__(self, root):
        UITabs.__init__(self, root)
