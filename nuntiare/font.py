...
class CairoFont(IFontManager):
    ...
    def get_text_height(text, page, cur_width, cur_height, style):
        ...
        if style.font_style == 'Normal':
            name_fd.set_style(CairoFont._get_style('NORMAL'))
        elif style.font_style == 'Italic':
            name_fd.set_style(CairoFont._get_style('ITALIC'))

        # Font weight
        if style.font_weight in ('Lighter', '100', '200'):
            name_fd.set_weight(CairoFont._get_weight('ULTRALIGHT'))
        elif style.font_weight == '300':
            name_fd.set_weight(CairoFont._get_weight('LIGHT'))
        elif style.font_weight in ('Normal', '400', '500'):
            name_fd.set_weight(CairoFont._get_weight('NORMAL'))
        elif style.font_weight in ('Bold', '600', '700'):
            name_fd.set_weight(CairoFont._get_weight('BOLD'))
        elif style.font_weight in ('Bolder', '800'):
            name_fd.set_weight(CairoFont._get_weight('ULTRABOLD'))
        elif style.font_weight == '900':
            name_fd.set_weight(CairoFont._get_weight('HEAVY'))
        ...
        pc = CairoFont._get_pango_context(cr)
        layout = CairoFont._get_layout(pc)

        layout.set_width(int(max_width * pango.SCALE))
        layout.set_font_description(name_fd)

        if style.text_align in ('General', 'Left'):
            layout.set_alignment(CairoFont._get_alignment('LEFT'))
        elif style.text_align == 'Right':
            layout.set_alignment(CairoFont._get_alignment('RIGHT'))
        elif style.text_align == 'Center':
            layout.set_alignment(CairoFont._get_alignment('CENTER'))

        # TODO
        # if style.text_justify:
        #    layout.set_justify(True)

        CairoFont._layout_set_text(layout, text)
        text_w, text_h = CairoFont._layout_get_width_height(layout)
    ...
    @staticmethod
    def _layout_set_text(layout, text):
        if _TYPE == 'GI':
    ...
    @staticmethod
    def _layout_get_width_height(layout):
        if _TYPE == 'GI':
