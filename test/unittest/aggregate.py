...
class AggregateTest(unittest.TestCase):
    ...
    def test_aggregate(self):
        '''
        Test a simple table (Tablix with just a TablixBody) and 
        aggregates Count, RunningValue, RowNumber, and Sum in diferent contexts.
        Test Grouping and sorting too.
        '''

        report_path = os.path.dirname(os.path.realpath(__file__))
        report_path = os.path.normpath(os.path.join(report_path, '..', 'report'))
        report = Report(os.path.join(report_path, 'northwind_orders.xml'))
        ...
        ## Austria ##
        self._ckeck_country_header(grid, 1, 7, 1, 'Austria', 305.00,
            4483.4, 994.72, 3488.68, 7, 3488.68, 498.38)
        self._ckeck_customer_header(grid, 2, 1, 'Ernst Handel', 305.00,
            4483.4, 994.72, 3488.68, 7, 3488.68, 498.38)
        self._ckeck_order_header(grid, 3, 1, 10258)
        self._ckeck_order_line(grid, 4, 1, 'Chang', 50.0,
            15.2, 760.0, 0.2, 152.0, 608.0, 1, 608.0, 608.0, 1)
        self._ckeck_order_line(grid, 6, 3, 'Mascarpone Fabioli', 6.0,
            25.6, 153.6, 0.2, 30.72, 122.88, 3, 1614.88, 538.29, 3)
        self._ckeck_order_footer(grid, 7, 3, 3, 3, 2018.6, 4483.4, 4483.4,
            1614.88, 1614.88, 1614.88, 10258)
        self._ckeck_order_header(grid, 8, 2, 10263)
        self._ckeck_order_line(grid, 10, 5, 'Longlife Tofu', 36.0,
            8.0, 288.0, 0.25, 72.0, 216.0, 2, 316.8, 158.4, 5)
        self._ckeck_order_line(grid, 12, 7, 'Pavlova', 60.0,
            13.9, 834.0, 0.25, 208.50, 625.5, 4, 1873.8, 468.45, 7)
        self._ckeck_order_footer(grid, 13, 4, 7, 7, 2464.8, 4483.4, 4483.4,
            1873.8, 3488.68, 3488.68, 10263)

        ## Brazil ##
        self._ckeck_country_header(grid, 21, 20, 3, 'Brazil', 229.0,
            4223.6, 260.4, 3963.2, 10, 3963.2, 396.32)
        self._ckeck_customer_header(grid, 22, 1, 'Hanari Carnes', 162.00,
            3257.8, 260.4, 2997.4, 6, 2997.4, 499.57)
        self._ckeck_order_header(grid, 23, 1, 10250)
        self._ckeck_order_line(grid, 24, 11, "Jack's New England Clam Chowder", 10.0,
            7.7, 77.0, 0.0, 0.0, 77.0, 1, 77.0, 77.0, 11)
        self._ckeck_order_line(grid, 26, 13, "Manjimup Dried Apples", 35.0,
            42.4, 1484.0, 0.15, 222.6, 1261.4, 3, 1552.6, 517.53, 13)
        self._ckeck_order_footer(grid, 27, 3, 3, 3, 1813.0, 3257.8, 4223.6,
        ...
        self._ckeck_customer_header(grid, 38, 3, "Wellington Importadora", 27.0,
            517.8, 0.0, 517.8, 2, 517.8, 258.9)
        self._ckeck_order_line(grid, 41, 20, "Perth Pasties", 15.0,
            26.2, 393.0, 0.0, 0.0, 393.0, 2, 517.8, 258.9, 20)
        self._ckeck_order_footer(grid, 42, 2, 2, 10, 517.8, 517.8, 4223.6,
            517.8, 517.8, 3963.2, 10256)

        ## Venezuela ##
        self._ckeck_country_header(grid, 200, 100, 13, 'Venezuela', 136.0,
            3635.9, 0.0, 3635.9, 9, 3635.9, 403.99)
        self._ckeck_customer_header(grid, 201, 1, "GROSELLA-Restaurante", 14.0,
            1101.2, 0.0, 1101.2, 2, 1101.2, 550.6)
        self._ckeck_order_line(grid, 203, 92, "Mozzarella di Giovanni", 4.0,
            27.8, 111.2, 0.0, 0.0, 111.2, 1, 111.2, 111.2, 92)
        self._ckeck_order_footer(grid, 205, 2, 2, 2, 1101.2, 1101.2, 3635.9,
            1101.2, 1101.2, 1101.2, 10268)
        self._ckeck_order_footer(grid, 218, 4, 4, 9, 1414.8, 1414.8, 3635.9,
            1414.8, 1414.8, 3635.9, 10283)

    def _ckeck_order_footer(self, grid, row, 
    ...
    def _ckeck_order_line(self, grid, row,
    ...
    def _ckeck_customer_header(self, grid, row,
    ...
    def _ckeck_country_header(self, grid, row,
            v1, v2, v3, v4, v5, v6, v7, v8, v9, v10):
